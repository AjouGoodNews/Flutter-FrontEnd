...
class NewsCategoryDetailScreen extends StatefulWidget {
  final String? categoryLabel;
  final String? category;
  const NewsCategoryDetailScreen({super.key, required this.categoryLabel, required this.category});

  @override
  State<NewsCategoryDetailScreen> createState() => _NewsCategoryDetailScreen();
}

class _NewsCategoryDetailScreen extends State<NewsCategoryDetailScreen> {
  ...
  Future<void> _fetchNewsData() async {
    final dss =widget.category;
    print('widget.category: $dss');
    if (widget.category == null) return;

    try {
      final newsData = await _categoryRepository.getNewsByCategory(widget.category!);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.categoryLabel ?? '', style: CustomTextStyle.body1), // AppBar 제목 설정
